...
(defn get-env-chime
  ...
  (if-let [chime-time (str/split (or (env :chime-time) "") #",")]
    (do (prn "Environment variable CHIME_TIME detected: " chime-time)
        (map #(Integer. %) chime-time))
    (do (prn "No-Environment variable CHIME_TIME, setting default time to 4 am")
        [4 0 0 0])))
...
(defn get-csv-files
  ...
  (if-let [csv-files (env :csv-files)]
    (do (prn "Restaurant inspections CSV files in environment variable CSV_FILES: "
             csv-files)
        (str/split csv-files #","))
    (let [default-csv-files ["ftp://dbprftp.state.fl.us/pub/llweb/1fdinspi.csv"
      ...
      (prn "No-Environment variable CSV_FILES, setting default csv files as "
           default-csv-files)
      default-csv-files)))
